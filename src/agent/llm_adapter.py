...
class LLMToolAdapter:
    ...
    def _call_litellm_model(
        ...
        if should_use_openai_responses(model, self._config):
            instructions, input_items = build_responses_request(
                messages,
                compat_mode=should_use_responses_compat(self._config, self._config.openai_base_url),
            )
            ...
            return self._parse_openai_responses(response, model)
    ...
    def _parse_openai_responses(self, response: Any, model: str) -> LLMResponse:
