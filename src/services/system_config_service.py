    ...
    def test_llm_channel(
        ...
        *,
        ...
        try:
            ...
            content = ""
            if resolved_protocol == "openai" and should_use_openai_responses(
                resolved_model,
                Config.get_instance(),
            ):
                instructions, input_items = build_responses_request(
                    [{"role": "user", "content": "Reply with OK"}],
                    compat_mode=should_use_responses_compat(Config.get_instance(), base_url.strip() or None),
                )
                extra_headers = None
                if base_url.strip() and "aihubmix.com" in base_url:
                ...
                content = extract_output_text(response).strip()
            else:
                import litellm
